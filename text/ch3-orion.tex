...
\begin{table}[]
	\begin{center}
		\begin{tabular}{lll}
			...
			Polarisation, $\phi$ (\unit{\degree})        & -42.2                                 &                           \\
			Normalised vector potential, $a_0$     & 7 - 10                               &                           \\
			...
			Polarisation, $\phi$ (\unit{\degree})          &        19.6                           &                            \\
			Normalised vector potential, $a_0$     & 7 - 10                               &                           \\
			Temporal FWHM, $t_\mathrm{FW}$ (fs)       & 470                                 &      1690                    \\
			Temporal envelope, $E_t$                          & \multicolumn{2}{l}{$E_t \sim e^{-(t-t_\mathrm{FW})^2\ln 2 /t_\mathrm{FW}^2}$} \vspace{0.15cm}\\ \hline \hline
			...
			Temporal resolution, $\Delta t$ (as)              & $\Delta x/11c$ = 2.51               & 0.00446         \\          			Macro-electrons per cell                       & \multicolumn{2}{l}{1000}                                                               \\
			...
			CVD density, $n_\mathrm{e}$                               & \num{1.05e24}                                   &  \\
			PMMA density, $n_\mathrm{e}$                               & \num{1.05e24}                                   &  \\
			Electron $x$ profile, $n(x)$                             & \multicolumn{2}{l}{$n_\mathrm{e} \text{ for $\lambda \le x \le 9\lambda$}$}                                               \\
			CVD carbon ion profile, $n_\mathrm{i}(x)$                                & \multicolumn{2}{l}{$n_\mathrm{i} = n(x)/6$}                                                    \vspace{0.15cm}\\ \hline \hline 
...
\end{table}
Figure \ref{fig:orionsimfigs} presents typical simulation results for the SP1 laser geometry.
\begin{figure}
	\centering
...
Then do theory vs parameters

Then do heating 

Then do full simulation



A sharp density gradient was applied 

Note that artificially increasing the plasma temperature to increase the Debye length and reduce numerical heating, while improving HHG accuracy, reduces hole boring accuracy since on sub-ps timescales, there is significant preplasma expansion. (Include plot demonstrating this)

Full large res sim

Discuss higher interp order.
...
(Include data point for circular pol sim)
