...
\begin{equation}
	T =m_\mathrm{e}c^2 \frac{a_0S}{2(1\pm\sin\theta)^2}
\end{equation}
...
Currently this model favours an angle of incidence of $\pi/2$, however this does not match with observations. The resolution comes by considering the maximum energy gained crossing the pseudocapacitor before interaction with the next peak of the laser pulse (and therefore launching of the electron bunch back towards the plasma bulk). If both electron bunch and laser pulse move at speed $c$, the electron bunch will reach the next peak of the laser pulse after propagating a distance $\lambda_\mathrm{L}/4$ along the $x$-axis. Adusting the limits of equation \ref{eq:zvp_Tp},

hmmm no this is wrong, think about wavefronts propagating at an angle. It would appear that for theta>45, the wavefront lambda/2 behind cannot catch up with teh electrons if they are travelling at speed c. If there are actually accelerating from rest and spend a reasonable amount of time below c then larger angles could catch up... think about this some more....


\begin{equation}
	T =
	\begin{cases}
		m_\mathrm{e}c^2 \frac{a_0S}{2(1\pm\sin\theta)^2} & x_\mathrm{p} \ge \lambda_\mathrm{L}/4\\
		m_\mathrm{e}c^2 \frac{a_0S}{2(1\pm\sin\theta)^2} -  & x_\mathrm{p} < \lambda_\mathrm{L}/4\\
	\end{cases}       
\end{equation}
...
Also mention alex's point about using 2D simulations.
