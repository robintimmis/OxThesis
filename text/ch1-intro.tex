...
The Debye length describes the extent to which a plasma can shield electromagnetic fields within a plasma and so remain quasineutral. Consider an infinite plasma where a line along $x = 0$ is held at some potential $\phi_0$ by some perfect system, then what would be the potential $\phi(x)$ perpendicular to that line? Assume that the ion-electron mass ratio is infinite, that is to say the ions are approximately immobile for the timescales under consideration. In \ac{1D}, Poisson's equation reads
