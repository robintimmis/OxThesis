...
% TODO reword the intro a little bit. Want to make more of the story. Perhaps discuss the recent Nobel prize and interest in attosecond physics, add an extra line on computing power, talk about GEMINI.
...
% TODO make a note that in a quantum plasma one actually does not want lots of particles in the Debye sphere
...
To ensure stability, or at least to minimise instability, there are several conditions which must be met. Naturally, the time step, $\Delta t$, and cell size ($\Delta x$x$\Delta y$x$\Delta z$) must adequately capture all interesting features of a given simulation. Typically such features are plasma wave oscilations,
\begin{equation}
...
Two experiments are discussed in this thesis, one performed on ORION, one planned on GEMINI. Both are PW class facilities. Their typical beam parameters are presented in Table \ref{tab:laser_params}.
\begin{table}[]
	\centering
	\begin{tabular}{lccc}
		\hline \hline
