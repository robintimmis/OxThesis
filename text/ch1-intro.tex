...
Consider an semi-infinite plasma existing for $x>0$, with electron density $n_e$ and ion density $n_e/Z$ with charge state $Z$\footnote{This description has direct relevance to the Zero Vector Potential mechanism which will be made clear later.}. Suppose the electron fluid is displaced into the plasma bulk a distance $\Delta r$ as in figure \ref{fig:introplasmafrequency}. 
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{../diagrams/intro_plasma_frequency}
	\caption{Diagram to illustrate the plasma frequency derivation.}
	\label{fig:introplasmafrequency}
\end{figure}
The displaced electrons will experience a restoring force, $-eE$, perpendicular to the surface due to the electron-ion charge imbalance. Applying the Lorentz force equation, the equation of motion for electrons on that surface is therefore
\begin{equation}
	m_e\frac{\mathrm{d}^2\Delta r}{\mathrm{d}t^2} = -eE.
\end{equation}
Applying Gauss' law as described in figure blah, the uncompensated charge leads to 
\begin{equation}
	E = \frac{en_e\Delta r}{\epsilon_0},
\end{equation}
therefore
\begin{equation}\label{eq:intro_sho}
	\frac{d^2\Delta r}{dt^2} = -\frac{e^2n_e}{m_e \epsilon_0}\Delta r.
\end{equation}
